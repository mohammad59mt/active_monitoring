...
class PSO():
    def old_initial(self, costFunc, initial_population, bounds, maxiter, verbose=False):
        ...
        i=0
        while i<maxiter:
            if verbose: print('iter: {i:>4d}, best solution: {fitness_best_g:10.6f}')
    ...
    def optimization(self,costFunc, bounds, max_itr, debug=False, optimal_fitness=None, acceptable_error=1e-5):
        ...
        for i in range(max_itr):
            ...
            if debug: print('iter: {i:>4d}, best solution: {self.fitness_best_g:4.2f}')
    ...
    def print_error(self,decimal_size=2):
        ...
        print('\nOptimization error:' + str(self.fitness_best_g))
...
def link_delay_measurement_and_comparison_PSO(array_of_delays, node_based_path_array, real_link_delays, init_pop_min=1, init_pop_max =10, min=1, max=100, pop_size = 100,max_itereration= 150, debug=False, save_to_file_dir=None):
    ...
    if save_to_file_dir is not None:
        with open (save_to_file_dir,"+w") as f:
            f.write(str_to_file)
    return max_difference,link_delay_error,link_real_and_measured_delay
